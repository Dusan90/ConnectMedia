...
const getPostsList = async () => {
    return await axiosInstance.get(`${url(GET_POST)}`)
}

const createPost = async ({ site,
    status,
    guid,
    title,
    link,
    ...
    author,
    content, }) => {
    const objective = { site, status, guid, title, link, image, timestamp, categories, description, author, content, }
...
const updatePostDetails = async ({ id, site, status, guid, title, link, image, timestamp, categories, description, author, content, }) => {
    const objective = { site, status, guid, title, link, image, timestamp, categories, description, author, content, }
    let dataforSend = Object.fromEntries(Object.entries(objective).filter(([_, v]) => v != null));
